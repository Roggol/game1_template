...
def display_room(room):
    ...
    # pass # The pass statement does nothing. Replace it with the body of your function.
    roomname = room["name"];
    description_of_room = room["description"];
    print("\n" + str(roomname).upper() + "\n\n" + str(description_of_room) + "\n");
...
def exit_leads_to(exits, direction):
    ...
    'Reception'
    """
    roomleads = exits[direction];
    return str(rooms[roomleads]["name"]);
...
def print_menu(exits):
    ...
    print("You can:\n")
    for key in exits:
        room_leads_to = exit_leads_to(exits, key)
        print_menu_line(str(key), room_leads_to);
...
def menu(exits):
    ...
    while True:
        ...
        # Read player's input
        direction_go = input("")
        if(direction_go == "QUIT"):
            Quit();
        # Normalise the input
        direction_go = normalise_input(direction_go);
        # Check if the input makes sense (is valid exit)
            # If so, return the player's choice
        if(is_valid_exit(exits, direction_go)):
            return direction_go;
